<?php
/**
 * a template for displaying a panel of testimonials
 */
?>
<section id="testimonials">
  <div class="my-20 mx-auto max-w-6xl flex justify-evenly space-x-12">
    <div class="flex space-x-6">
      <div class="w-36"><img class="w-full h-auto rounded-full" src="/wp-content/themes/tailkick/images/tk-testimonial-1.jpg" /></div>
      <div class="space-y-2">
        <h4 class="font-bold text-xl">“It’s so cute I can’t handle it...”</h4>
        <p>... Interdum et malesuada fames ac ante ipsum primis in faucets.</p>
        <p>LINA MORRIS</p>
      </div>
    </div>
    <div class="flex space-x-6">
      <div class="w-36"><img class="w-full h-auto rounded-full" src="/wp-content/themes/tailkick/images/tk-testimonial-2.jpg" /></div>
      <div class="space-y-2">
        <h4 class="font-bold text-xl">“I snagged this lil’ gem while in LA...” </h4>
        <p>... Interdum et malesuada fames ac ante ipsum primis in faucets.</p>
        <p>JOHNNA MYERS</p>
      </div>
    </div>
    <div class="flex space-x-6">
      <div class="w-36"><img class="w-full h-auto rounded-full" src="/wp-content/themes/tailkick/images/tk-testimonial-3.jpg" /></div>
      <div class="space-y-2">
        <h4 class="font-bold text-xl">“I have the best conversations when...”</h4>
        <p>... Interdum et malesuada fames ac ante ipsum primis in faucets.</p>
        <p>JESSICA SIMS</p>
      </div>
    </div>
  </div>
</section>
